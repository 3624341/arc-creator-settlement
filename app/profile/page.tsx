...
export default function ProfilePage() {
  ...
  return <Shell>
    ...
    <section className="mt-8 grid gap-6 lg:grid-cols-3">
      <div className="rounded-[2rem] border border-arc-line bg-white/80 p-6 lg:col-span-2"><h2 className="text-2xl font-black">Created Jobs</h2><p className="mt-2 text-sm text-arc-muted">Delete local drafts or hide deployed escrows from this workspace. Onchain records remain verifiable on Arc.</p>{createdContracts.length ? <div className="mt-5 grid gap-3">{createdContracts.map((contract) => <article key={contract.id} className="flex items-center gap-3 rounded-2xl border border-arc-line p-4 hover:bg-arc-bg"><Link href={`/contracts/${contract.escrowAddress ?? contract.id}`} className="min-w-0 flex-1"><p className="font-black">{contract.title}</p><p className="mt-2 text-sm text-arc-muted">{contract.status} · {contract.applications?.length ?? 0} applications</p></Link><div className="flex shrink-0 items-center gap-3"><span className="text-sm font-bold text-arc-muted">{contract.totalUsdc} USDC</span><button type="button" className="rounded-full border border-red-200 px-3 py-2 text-xs font-black text-red-600 hover:bg-red-50" onClick={() => removeContract(contract)}>{isDeployedContract(contract) ? "Hide" : "Delete"}</button></div></article>)}</div> : <p className="mt-4 text-sm text-arc-muted">No jobs created yet. <Link className="font-black text-arc-purple" href="/contracts/create">Create a contract →</Link></p>}</div>
