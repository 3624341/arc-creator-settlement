...
| 02 shared applications | `lib/marketplace-store.ts` uses localStorage only | Add Supabase-backed API/schema with local fallback | Implemented in code; production activation requires Supabase schema/env |
...
| 10 receipt discovery | Direct receipt works; list depends on API/local hashes | Improve public receipt index path | Implemented in code; production activation requires Supabase schema/env |
